...
# ---------------------------------------------------------
CRIME_FILE_NAME = "Fallzahlen_2024.csv" 

# ---------------------------------------------------------
# 1. 설정 및 API 키 로드
# ---------------------------------------------------------
st.set_page_config(layout="wide", page_title="베를린 풀코스 가이드 (OSM)")
...
st.caption("OpenStreetMap & 2024년 최신 통계")
...
with tab1:
    ...
    # 1. 범죄 지도
    if show_crime:
        crime_df = load_and_process_crime_data(CRIME_FILE_NAME)
...
# =========================================================
with tab4:
    st.header("📊 베를린 범죄 데이터 대시보드 (2024)")
    st.caption(f"데이터 원본: {CRIME_FILE_NAME}")

    raw_df = load_crime_data_raw(CRIME_FILE_NAME)

    if not raw_df.empty:
        c_filter1, c_filter2 = st.columns(2)
        with c_filter1:
            st.info("📅 2024년 데이터 분석")
        with c_filter2:
